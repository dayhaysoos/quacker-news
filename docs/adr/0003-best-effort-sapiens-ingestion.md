...
Convex should check for SAPIENS ingestion hourly, but the effective interval is
controlled at runtime with `SAPIENS_INGESTION_ENABLED` and
`SAPIENS_INGESTION_INTERVAL_HOURS`. Missing or invalid interval values default
to 6 hours and are clamped to 1 through 24 hours. These controls are backend
configuration only and must not become public UI, dashboards, debug controls, or
